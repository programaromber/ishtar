...
export class ParticipacaoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.participacaoService
      ...
      .subscribe(
        (res: HttpResponse<IParticipacao[]>) => this.paginateParticipacaos(res.body, res.headers),
  ...
  protected paginateParticipacaos(data: IParticipacao[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.participacaos = data;
  }
